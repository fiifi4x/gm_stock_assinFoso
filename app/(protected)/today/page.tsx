...
export default function TodayPage() {
  ...
  const violations = useMemo(() => {
    if (!flags) return []
    const list: { label: string; count: number; days: number | null; href: string }[] = []
          ...
          <div className="divide-y divide-gray-100">
            {violations.map(v => (
              <Link key={v.href} href={v.href}
                className="flex items-center justify-between py-2 hover:bg-gray-50 -mx-1 px-1 rounded transition">
                ...
                <span className="text-xs text-blue-600 font-semibold shrink-0 ml-2">Fix →</span>
              </Link>
            ))}
          </div>
        )}
